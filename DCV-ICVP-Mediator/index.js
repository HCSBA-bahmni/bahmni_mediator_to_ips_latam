...
/* =========================================================
 * 1) VHL — emitir HC1 desde Bundle (como tenías)
 *    POST /vhl/_generate
 * =======================================================*/
  ...
  try {
    ...
    if (typeof bundle === 'string') {
      try {
        ...
        /* si viene texto irreconocible, lo dejamos tal cual */
      }
    }
    ...
    const resp = await axios.post(VHL_ISSUANCE_URL, payload, {
      headers: buildUpstreamJsonHeadersVHL(),
      responseType: 'text',
      timeout: 30000
    });
...
/* =========================================================
 * 2) VHL — validar HC1 y obtener URL (como tenías)
 *    POST /vhl/_validate
 * =======================================================*/
app.post('/vhl/_validate', async (req, res) => {
  try {
    ...
    const upstream = await axios.post(
      VHL_VALIDATION_URL,
      { qrCodeContent },
      { headers: buildUpstreamJsonHeadersVHL(), responseType: 'json', timeout: 30000 }
    );
...
/* =========================================================
 * 3) VHL — resolver manifest (como tenías)
 *    POST /vhl/_resolve
 * =======================================================*/
app.post('/vhl/_resolve', async (req, res) => {
  try {
    ...
    const recipient = VHL_RECIPIENT || 'Bahmni Client';
    const passcode = VHL_PASSCODE || '1234';
